...
//   Versao 1.40b
//   DATA: 15/04/20
//   HORA: 01:00    revisão e alterações Gustavo - ajuste dos parâmetros com Encoder
//
//   ANALISTA
...
char  versao_SOLAR    =      " Versao 1_40b";
...
void LCD_Inicial (char versao_SOLAR)    // Tela inicial no display LCD
{
  ...
  lcd.print("*   Versao 1.40b   *");                      // mostra no LCD
  ...
  switch (keypad.getState())                  // verifica se algum botão foi pressionado
  {
    case PRESSED:                             // identifica o botão pressionado
      ...
      if (key == 'C')                         // Botao C
      {
        ...
        LCD_mostra_Parametros ();             // linha 496
      }
      //-------------------------------------------------------------------------------
      if (key == 'D')                         // Botao D
      {
